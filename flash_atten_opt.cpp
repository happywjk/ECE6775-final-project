//===============================================================
// FlashAttention - Memory Simulation Version (int8 stream, float compute)
//===============================================================
  ...
  for (int b = 0; b < BATCH_SIZE; ++b) {
    for (int t = 0; t < CONTEXT_LENGTH; ++t) {
      for (int c = 0; c < THREE_H; ++c) {
        #pragma HLS PIPELINE II=1
        data_t x = input_data[idx++]; // Reading from Main Memory simulation
      ...
      for (int tq = 0; tq < CONTEXT_LENGTH; ++tq) {
        ...
        for (int tk0 = 0; tk0 < CONTEXT_LENGTH; tk0 += BLOCK_T) {
          ...
          for (int tj = 0; tj < BLOCK_T; ++tj) {
              ...
              for (int d = 0; d < HEAD_DIM; ++d) {
                  #pragma HLS UNROLL
                  dot += (acc_t)Q[b][h][tq][d] * (acc_t)K[b][h][tk][d];
              }
              scores_tile[tj] = (float)dot * scale;
          }
          ...
          for (int d = 0; d < HEAD_DIM; ++d) {
              ...
              for(int tj=0; tj<BLOCK_T; ++tj) {
                  #pragma HLS UNROLL
                  int tk = tk0 + tj;
                  weighted_sum += exp_vals[tj] * (float)V[b][h][tk][d];
              }
...
static void compute_engine(data_t input_mem[IN_ELEMS], float output_mem[OUT_ELEMS]) {
